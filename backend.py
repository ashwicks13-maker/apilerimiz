...
def clean_text(text):
    ...
    text = str(text)
    # Gerçek kontrol karakterlerini temizle
    text = text.replace('\n', ' ').replace('\r', ' ').replace('\t', ' ')
    # Literal \n dizisini temizle (veritabanında yazılmışsa)
    text = text.replace('\\n', ' ').replace('\\r', ' ').replace('\\t', ' ')
    return text.strip()
...
def vergi_adi():
    ...
    if ad and soyad:
        sql = "SELECT * FROM kisiler WHERE fullname LIKE ? AND fullname LIKE ? LIMIT 50"
        params = (f"%{ad}%", f"%{soyad}%")
    else:
        aranan = ad if ad else soyad
        sql = "SELECT * FROM kisiler WHERE fullname LIKE ? LIMIT 50"
        params = (f"%{aranan}%",)
...
def vergi_tc():
    ...
    conn = get_vergi_connection()
    cur = conn.cursor()
    sql = "SELECT * FROM kisiler WHERE identity LIKE ? LIMIT 50"
